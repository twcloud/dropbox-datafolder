...
namespace wrapper {
	...
	interface IWikiHandlerConstructor{
		new(): IWikiHandler;
	}
	...
	type MODES = "iframe" | "blob" | "replace";
	...
	class TwitsLoader {
		...
		constructor(
			options: {[K in "type" | "path" | "user" | "hash" | "method"]: string}
		) {
			this.type = options.type;

			this.status = new StatusHandler("");
			this.wiki = new WikiHandler({
				status: this.status,
				type: options.type as any,
				mode: options.method as any
			})
	...
	class WikiHandler implements IWikiHandler {
		readUserInfo(): Promise<IUserInfo> {
		...
		constructor(options: {
			...
			this.type = options.type;
			if (this.type !== "apps" && this.type !== "full") throw "type must be either apps or full"
...
` + '</' + 'script>\n',
	...
	if (location.protocol.indexOf('http') === 0) {
		...
		document.addEventListener("DOMContentLoaded", function (event) {
			...
			new TwitsLoader(accessType, options);
		}, false);
